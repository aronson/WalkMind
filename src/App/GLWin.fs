...
let create (options: GlWindowOptions) : GlWindowCtx =
    let windowOptions =
        new WindowOptions(
            options.IsVisible,
            new Silk.NET.Maths.Vector2D<int>(options.Position.X, options.Position.Y),
            new Silk.NET.Maths.Vector2D<int>(options.Size.Width, options.Size.Height),
    ...
    let logger =
        ...
        | Some logger -> logger
        | None -> new NullLogger<obj>() :> Microsoft.Extensions.Logging.ILogger
